...
const PricingTable = ({ data }) => {
  const renderTableCell = row => {
    if (typeof row.pro === 'boolean') {
      return <Iconify fontSize={20} icon={row.pro ? 'mdi:check-circle' : 'mdi:close-circle'} />
    } else {
                    ...
                    >
                      {head.isPro ? (
                        ...
                        >
                          ...
                          {head.isPro ? <Iconify icon='mdi:star-circle' /> : null}
                        </Box>
                      ) : (
                        <Typography noWrap sx={{ fontSize: '.75rem', fontWeight: 600, letterSpacing: '.17px' }}>
                          {head.title}
            ...
            <TableBody>
              {data.pricingTable.rows.map((row, index) => (
                <TableRow key={index}>
                  ...
                  >
                    <Iconify fontSize={20} icon={row.starter ? 'mdi:check-circle' : 'mdi:close-circle'} />
                  </TableCell>
                  <TableCell
                    align='center'
                  ...
                  >
                    <Iconify fontSize={20} icon={row.enterprise ? 'mdi:check-circle' : 'mdi:close-circle'} />
                  </TableCell>
                </TableRow>
              ))}
