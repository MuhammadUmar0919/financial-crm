...
const HelpCenterArticle = ({ articles, activeArticle, activeSubcategory }) => {
  ...
  const handleChange = (event, newValue) => {
    setIsLoading(true)
    navigate({ pathname: `/general-settings/help-center/${navigate.query.category}/${navigate.query.subcategory}/${newValue}` })
      ...
      <Card>
        <CardContent>
          <Button
            ...
            to={`/general-settings/help-center/${navigate.query.category}/${navigate.query.subcategory}`}
          >
            Back to Categories
          </Button>
          ...
          <Box sx={{ mt: 6, mb: 6.5, display: 'flex', alignItems: 'center' }}>
            <CustomAvatar skin='light' variant='rounded' color='secondary' sx={{ mr: 3 }}>
              <Iconify icon={activeSubcategory.icon} />
            </CustomAvatar>
          ...
          <div>
            ...
            <div>
              <Button variant='outlined' sx={{ mr: 2.5, p: 1.5, minWidth: 32 }}>
                <Iconify fontSize={18} icon='mdi:thumbs-up-outline' />
              </Button>
              <Button variant='outlined' sx={{ p: 1.5, minWidth: 32 }}>
                <Iconify fontSize={18} icon='mdi:thumbs-down-outline' />
              </Button>
            </div>
          </div>
          <Box sx={{ display: 'flex' }}>
