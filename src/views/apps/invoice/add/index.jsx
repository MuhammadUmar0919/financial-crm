...
import { FormControl, FormLabel, Select, Stack, Switch } from '@mui/material';
...
import { selectCreatedItem, selectListItems, selectUpdatedItem } from '@/Redux/crud/selectors';
...
const AddWrapper = () => {
  ...
  const { create, currentData } = useSelector((state) => state.config);
  const invoiceNumber = Math.max(0, ...dataSource.map((i) => i.id || 0)) + 1;
  const { isLoading: loading, isSuccess } = useSelector(
    create ? selectCreatedItem : selectUpdatedItem
  );
  ...
  const {
    ...
    defaultValues: create ? { ...initialState, id: invoiceNumber } : currentData,
  });
  ...
  const onSubmit = async (data) => {
    try {
      create
        ? dispatch(crud.create({ entity: 'invoices', jsonData: data }))
        : dispatch(crud.update({ entity: 'invoices', id: data.protId, jsonData: data }));
    } catch (error) {
  ...
  React.useEffect(() => {
    if (isSuccess) {
      ...
      dispatch(crud.list({ entity: 'invoice' }));
    }
  }, [isSuccess]);
                    ...
                    <Grid container spacing={3}>
                      <Grid item xs={12} xl={4}>
                        <FormControl fullWidth>
                          <FormLabel htmlFor="invoice-id">Invoice</FormLabel>
                          <Controller
                            name="id"
                            control={control}
                            render={({ field }) => (
                    ...
                    render={({ field: { value, onChange } }) => (
                      ...
                      >
                        {statusData.map(({ statusName }, idx) => (
                          <MenuItem autoWidth key={idx} value={statusName}>
                            <Label
              ...
              <CardContent>
                <Grid container spacing={4}>
                  <Grid
                    ...
                    sx={{
                      my: { lg: 0 },
                      textAlign: { xs: 'end', lg: 'start', md: 'start' },
                      mt: 2,
                    }}
                  >
                    <Button
                ...
                <Grid container>
                  ...
                  <Grid item xs={12} sm={3} sx={{ mb: { sm: 0, xs: 4 }, order: { sm: 2, xs: 1 } }}>
                    ...
                    <CalcWrapper>
                      ...
                      <Typography
                        variant="body2"
                        sx={{ fontWeight: 600, color: color.errorLight, lineHeight: '.25px' }}
                      >
                        {shipping ? `-$${shipping}` : '-'}
                      </Typography>
                    </CalcWrapper>
