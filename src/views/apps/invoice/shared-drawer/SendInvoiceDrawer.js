...
const SendInvoiceDrawer = ({ open, toggle, onSubmit }) => {
  const handleSubmit = () => {
    // toggle()
    onSubmit()
      ...
      <Box sx={{ p: 5 }}>
        ...
        <Box sx={{ mb: 6 }}>
          <CustomChip
            ...
            sx={{ borderRadius: '5px' }}
            icon={<Iconify icon='mdi:attachment' fontSize={20} />}
          />
        </Box>
        <div>
          <Button size='large' variant='contained' onClick={handleSubmit} sx={{ mr: 4 }}>
            Send
          </Button>
