...
export default function StatusForm({ data, close }) {
  const dispatch = useDispatch();
  const {statusData, entity} = config;
  const [open, setOpen] = React.useState(false);
  const isDesktop = useResponsive('up', 'lg', 'sm', 'xs');
  const { isLoading: loading, isSuccess } = useSelector(selectUpdatedItem);
  ...
  const onSubmit = async (values) => {
    try {
      ...
      await dispatch(crud.update({ entity, protId: value.protId, jsonData: value }));
      await TimeSleep();
    } catch (error)  {
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogContent>
            <Grid container spacing={4}>
              ...
              {status === 'rejected' && entity === 'clients' && (
                <Grid item xs={12}>
                  <Controller
                    control={control}
                    name="rejectedReason"
