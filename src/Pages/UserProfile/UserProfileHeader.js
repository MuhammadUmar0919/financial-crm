...
const UserProfileHeader = () => {
  ...
  const { coverImg, profileImg, fullName, roleIcon, roleName, address, joiningDate} = token;
  // useEffect(() => {
  //   axios.get('/pages/profile-header').then(response => {
  //     setData(response.data)
  //   })
  // }, [])
  const designationIcon = "mdi:fountain-pen-tip" || 'mdi:briefcase-outline'
    ...
    <Card>
      ...
      >
        ...
        >
          <Box sx={{ mb: [6, 0], display: 'flex', flexDirection: 'column', alignItems: ['center', 'flex-start'] }}>
            ...
            >
              <Box sx={{ mr: 4, display: 'flex', alignItems: 'center', '& svg': { mr: 1, color: 'text.secondary' } }}>
                ...
                <Typography sx={{ color: 'text.secondary', fontWeight: 600 }}>{sentenceCase(roleName)}</Typography>
              </Box>
              <Box sx={{ mr: 4, display: 'flex', alignItems: 'center', '& svg': { mr: 1, color: 'text.secondary' } }}>
          ...
          <Button variant='contained' startIcon={<Iconify icon='mdi:account-check-outline' fontSize={20} />}>
            Connected
          </Button>
        </Box>
      </CardContent>
