...
const defaultColumns = [
  {
    ...
    minWidth: 80,
    headerName: '#',
    renderCell: ({ row }) => <StyledLink to={`/apps/invoice/preview/${row.id}`}>{`#${row.id}`}</StyledLink>
  },
  {
    flex: 0.1,
    minWidth: 80,
      ...
      return (
        ...
        >
          <CustomAvatar skin='light' color={color} sx={{ width: '1.875rem', height: '1.875rem' }}>
            <Iconify icon={invoiceStatusObj[invoiceStatus].icon} fontSize='1rem' />
          </CustomAvatar>
        </Tooltip>
      )
    }
    ...
    ...defaultColumns,
    {
      ...
      renderCell: ({ row }) => (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          ...
          <Tooltip title='View'>
            <IconButton size='small' component={Link} to={`/apps/invoice/preview/${row.id}`}>
  ...
  return (
    <Card>
      ...
      <CardContent>
        ...
        >
          <Button component={Link} variant='contained' to='/invoice/create' startIcon={<Iconify icon='mdi:plus' />}>
            Create Invoice
          </Button>
          <Box
            sx={{
